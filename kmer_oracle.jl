...
# TODO: the "evaluate" functions could be a lot faster by avoiding push!
...
function train_and_plot(data::Datafile, hyper::Hyperparams, visu::Plotparams; device::Function=cpu, nohup::Bool=false)
    ...
    # Splitting into sets
    i_train, e_train, i_test, e_test = split_kmer_data(all_kmers, all_counts, 10, hyper.use_training_set)
...
path = "/u/jacquinn/graphs_fixed_loss/0.00001_hex_3000k_neurones/"

# Loading data and saving the model
data = Datafile(paths=files, datasets=datasets, model_path=path, save_every=5, save_model=true)

# Hyperparams struct
hyper = Hyperparams(training_rate=0.00001, l2_multiplier=1e-5, use_log_counts=true, batchsize=2048)
